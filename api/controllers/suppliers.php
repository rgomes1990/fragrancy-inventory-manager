...
function handleRequest(array $user, ?string $id): void {
    ...
    $crud = new CrudHelper('suppliers', $user['username'], $tenantId);
    ...
    if ($method === 'GET' && $id) {
        $r = $crud->getById($id);
        $r ? jsonResponse($r) : errorResponse('Fornecedor nao encontrado', 404);
    }
    if ($method === 'POST') {
    ...
    if ($method === 'DELETE' && $id) {
        $crud->delete($id) ? jsonResponse(['success' => true]) : errorResponse('Fornecedor nao encontrado', 404);
    }

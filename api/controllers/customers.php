...
function handleRequest(array $user, ?string $id): void {
    ...
    $crud = new CrudHelper('customers', $user['username'], $tenantId);
    ...
    if ($method === 'GET' && $id) {
        $r = $crud->getById($id);
        $r ? jsonResponse($r) : errorResponse('Cliente nao encontrado', 404);
    }
    if ($method === 'POST') {
    ...
    if ($method === 'DELETE' && $id) {
        $crud->delete($id) ? jsonResponse(['success' => true]) : errorResponse('Cliente nao encontrado', 404);
    }
